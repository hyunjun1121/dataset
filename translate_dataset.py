#!/usr/bin/env python3
"""
Dataset translation script using NLLB-200-3.3B model.
Translates processed JSON files to multiple languages.
"""
import json
import os
import torch
...
class DatasetTranslator:
    ...
    def translate_file(self, input_file, output_file, target_lang_code, batch_size=8):
        """
        Translate all texts in a JSON file.
        ...
        print(f"Translating {len(texts)} texts to {target_lang_code}")
    ...
    def translate_all_processed_files(self, processed_dir, output_dir, target_languages):
        """
...
def main():
    parser = argparse.ArgumentParser(description="Translate dataset using NLLB-200-3.3B")
    parser.add_argument("--processed_dir", default="processed_data", 
                       help="Directory containing processed JSON files")
    parser.add_argument("--output_dir", default="translated_data", 
    ...
    # Translate all files
    translator.translate_all_processed_files(
        args.processed_dir,
        args.output_dir,
        args.languages
    )
